...
function import_data() {
	...
	do
		...
		if [ -n "$g_checklist__name" ] && [ -n "$g_checklist__update_time" ] && [ -n "$g_checklist__stb_sn" ]; then
			...
			#echo ""
			#echo "${g_checklist_list__name}/${g_checklist_list__update_time}/${g_checklist_list__checklist_uid}/${g_checklist_list__value}"
			...
			echo "UPDATE $db_table_g_checklist_list SET checklist_uid = '$g_checklist_list__checklist_uid' where uid = $iter;"
			echo "UPDATE $db_table_g_checklist_list SET value = '$g_checklist_list__value' where uid = $iter;"
		fi
		...
		if [ -n "$g_home_list__name" ] && [ -n "$g_home_list__update_time" ] && [ -n "$g_home_list__g_home_uid" ]; then
			...
			echo "UPDATE $db_table_g_home_list SET g_home_uid = '$g_home_list__g_home_uid' where uid = $iter;"
		fi
		...
		if [ -n "$g_surgery__name" ] && [ -n "$g_surgery__update_time" ] && [ -n "$g_surgery__g_main_home_uid" ]; then
                        ...
                        echo "UPDATE $db_table_g_surgery SET g_main_home_uid = '$g_surgery__g_main_home_uid' where uid = $iter;"
                fi
