...
*/
    ...
    async schedule_main(json){
        try {
            ...
            'from g_main inner join g_main_list on g_main.main_list_order = g_main_list.order where g_main.stb_sn = ?', stb_sn, (err, stb)=> {
                // console.log(stb);
                if(err) console.log(err);
                if(stb.length != 0){
    ...
    async schedule_home(json){
        try {
            ...
            db.query('select g_home.stb_sn, g_home.name as home_name, g_home.update_time as home_update_time, g_home_list.name as list_name, '+
            'g_home_list.update_time as list_update_time, g_home_list.order as list_order, g_home_list.start as list_start, g_home_list.end as list_end '+
            'from g_home inner join g_home_list on g_home.home_list_order = g_home_list.order where g_home.stb_sn = ?', stb_sn, (err, stb)=> {
                console.log(stb);
                if(err) console.log(err);
                if(stb.length != 0){
    ...
    async schedule_layout(json){
        try {
            ...
            'g_layout_list_define.height, g_layout_list_define.contents from g_layout inner join g_layout_list_layout on g_layout.order = g_layout_list_layout.order '+
            'inner join g_layout_list_define on g_layout.order = g_layout_list_define.order where g_layout.stb_sn = ?', stb_sn, (err,stb)=> {
                // console.log(stb);
                if(err) console.log(err);
                if(stb.length != 0){
    ...
    async schedule_checklist(json){
        try {
            ...
            db.query('select g_checklist.stb_sn, g_checklist.name as checklist_name, g_checklist.update_time as checklist_update_time, g_checklist_list.name as list_name, '+
            'g_checklist_list.update_time as list_update_time, g_checklist_list.order as list_order '+
            'from g_checklist inner join g_checklist_list on g_checklist.checklist_list_order = g_checklist_list.order where g_checklist.stb_sn = ?', stb_sn, (err, stb)=> {
                console.log(stb);
                if(err) console.log(err);
                if(stb.length != 0){
                    ...
                    let checklist = [];
                    for(let i=0; i<stb.length; i++){
                        let checklist_list = new Object();
                        checklist_list.name = stb[i].list_name;
                        checklist_list.update_time = stb[i].list_update_time;
                        checklist_list.order = stb[i].list_order;
                        checklist_list.start = stb[i].list_start;
                        checklist_list.end = stb[i].list_end;
                        checklist.push(checklist_list);
                    }
    ...
    async schedule_media(json){
        try {
            ...
            'from g_media inner join g_media_list on g_media.media_list_order = g_media_list.order where g_media.stb_sn = ?', stb_sn, (err, stb)=> {
                console.log(stb);
                if(err) console.log(err);
                if(stb.length != 0){
